...
(function() {
  ...
  let newBody = element;
  let scripts = oldBody.getElementsByTagName("script");

  // Remove all child nodes from the body, except for scripts
  while (oldBody.firstChild) {
    if (oldBody.firstChild.tagName !== "SCRIPT") {
      oldBody.removeChild(oldBody.firstChild);
    } else {
      oldBody.firstChild.remove();
    }
  }

  // Append the new body element
  oldBody.appendChild(newBody);

  // Re-add the scripts to the body
  for (let i = 0; i < scripts.length; i++) {
    oldBody.appendChild(scripts[i]);
  }
}

let element = document.querySelector('.section-size4')
element.style.paddingTop = '0'
replaceBody(element);
document.querySelector('.row').style.display='none'
document.querySelectorAll('.column')[1].style.display='none'
document.querySelectorAll('.column')[2].style.margin='0'
document.querySelector(".text-body4").parentElement.style.display='none'
